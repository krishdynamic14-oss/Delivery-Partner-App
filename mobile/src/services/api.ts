...
export async function fetchOrders(district: string, token?: string): Promise<DeliveryOrder[]> {
  if (GAS_URL) return request<DeliveryOrder[]>('orders.byDistrict', { district }, token);
